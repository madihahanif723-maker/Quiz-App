...
function login() {
    ...
    if (userName && rollNumber) {
        ...
        Swal.fire({
            title: "⚠️ Wait a moment!",
            text: "Please enter your name and roll number to begin ✍️",
            icon: "warning",
...
function startTimer() {
    timer = setInterval(function () {
        ...
        var usedMin = Math.floor(timeUsed / 60);
        var usedSec = timeUsed % 60;


        document.getElementById("timer").innerHTML =
            `⏱️ Left: ${minutes}:${seconds < 2 ? "0" : ""}${seconds} 
             | ⏳ Used: ${usedMin}:${usedSec < 2 ? "0" : ""}${usedSec}`;
...
function displayContainer() {
    ...
    if (index >= QuizData.length) {
        ...
        Swal.fire({
            ...
            html: `Your Score: <b>${score}/${QuizData.length}</b> 🎉`,
            icon: "success",
            background: "#1e293b",
            color: "#e2e8f0",
            confirmButtonColor: "#7c3aed",
    ...
    quizcontainer.innerHTML = `
        ...
        <div class="d-flex justify-content-between mt-3">
            <button class="btn butcol btn-nav" onclick="previousQuiz()">⬅️Previous</button>
            <button id="nextBtn" class="btn butncol btn-nav" onclick="nextQuiz()">Next➡️</button>
        </div>
    `;
